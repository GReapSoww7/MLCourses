...
-logistic regression predictions: sign of raw model output
    -like the definite class predictions (which come from the raw model output [- or +])

-logistic regression probabilities: "squashed" raw model output
    -value between 0->1 (sigmoid function does this for us)
    -the farther away we move from the boundary > we are more confident in our predictions the farther away from the boundary we are.
